...
首次启动后或执行 `恢复出厂设置` 命令后，您将看到下面的屏幕。<br/>
  ...
  * 如果无法扫码连接，请到设备的 `设置` 菜单中手动连接热点。以 `iPhone` 为例：
    * 在你的主屏幕上，前往 `设置`-> `无线局域网`。
...
* 连接上 AP 后，理应有 `已连接（需登录/认证）` 的提示，并且点击会进入Web UI配置界面。有时因为网络原因，需要等待10~20秒, 如果等待后仍然无法进入Web UI配置界面, 可以在浏览器中手动输入 [192.168.4.1](http://192.168.4.1) 访问Web UI配置界面。
...
可以打开/关闭 “预览模式”开关来显示/隐藏“预览”按钮，点击“预览此GIF”可以使 Panda Knomi进入预览模式，屏幕上将显示“预览模式”字样，在屏幕上可查看到自定义 GIF 的效果，若想退出预览模式，请关闭“预览模式”开关，或者直接跳转到除“主题”界面之外的其他界面。
  自定义GIF注意
