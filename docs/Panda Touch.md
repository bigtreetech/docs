...
如果您的任何P1P/S打印机未设置为“仅局域网”模式，则您需要通过panda touch启用与它们的云连接，以控制以下功能：
*加热
*开始打印
*暂停打印
*停止打印

按照以下步骤启用与需要它的打印机的云连接。
...
* 如果您找不到登录凭据，请参阅以下部分：[我找不到我的Bambu实验室登录凭据](#我找不到我的Bambu实验室登录凭据)
...
* 我们建议只有当您的所有机器都使用云模式运行时，才转换所有打印机。 如果使用仅局域网模式，则最好手动将每台机器设置为云模式，如中所述 [如何改变PandaTouch与打印机之间的通信方式](#如何改变PandaTouch与打印机之间的通信方式).
...
* 一旦打印机在云模式下工作，您将在多打印机视图中注意到与其卡片关联的绿色云。云表示帐户登录状态。
...
##### 如何改变PandaTouch与打印机之间的通信方式
* 导航到打印机/组卡页面。
* 找到要启用或禁用云连接的打印机。
* 向下滚动到该打印机的“编辑”按钮。
* 选择“启用云”将打印机设置为关闭连接模式。
> 提示
...
要在Panda Touch上启用云连接，您需要使用下载Bambu Handy进行操作：
...
> NOTE: 更新V1.0.2以上版本时需要同时更新两个文件:一个是固件，另一个是img文件. 这将有助于我们在未来为Panda Touch添加更多的语言，请确保同时下载.bin和.img文件.
